...
protocol SabycomService {
    ...
    func show(on viewController: UIViewController, pushInfo: [AnyHashable: Any]?)
...
class SabycomServiceImpl: SabycomService {
    ...
    func show(on viewController: UIViewController, pushInfo: [AnyHashable: Any]?) {
        configureSabycom()
        SabycomSDK.show(on: viewController, pushInfo: pushInfo)
    }
...
//            #endif
            ...
            print("Нет токена для уведомлений")
        }
    }
}
