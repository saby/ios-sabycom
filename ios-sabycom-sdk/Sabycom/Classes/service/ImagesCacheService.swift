...
class ImagesCacheServiceImpl: ImagesCacheService {
    ...
    func saveImage(_ image: UIImage, url: URL) {
        ...
        guard let data = image.pngData() else {
            logger.log("Не удалось создать файл картинки в формате png")
            return
        }
        ...
        do {
            ...
            logger.log("Ошибка записи файла на диск по пути \(path)")
        }
    }
    ...
    func loadImage(for url: URL) -> UIImage? {
        ...
        do {
            ...
            logger.log("Ошибка загрузки файла с диска по пути: \(path)")
        }
